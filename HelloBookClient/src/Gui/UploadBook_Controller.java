...
public class UploadBook_Controller extends Base_Controller implements Initializable {
	...
	public void saveAction() {// 책 업로드
		if (tf_Title.getText().length() == 0 || tf_Author.getText().length() == 0
				|| tf_Publisher.getText().length() == 0 || cb_Genre.getSelectionModel().getSelectedItem() == null
				|| condition.getSelectedToggle() == null || tf_FullPrice.getText().length() == 0
				|| tf_SalePrice.getText().length() == 0 || tf_LendPrice.getText().length() == 0
				|| ta_Introduction.getText().length() == 0) {
